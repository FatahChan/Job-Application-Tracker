...
async function getApplications(): Promise<Document<JobApplicationType>[]> {
  return (await databases.listDocuments<Document<JobApplicationType>>(DATABASE_ID, JOB_APPLICATION_COLLECTION_ID))
    .documents;
}

async function getApplication(id: string): Promise<Document<JobApplicationType>> {
  return databases.getDocument<Document<JobApplicationType>>(DATABASE_ID, JOB_APPLICATION_COLLECTION_ID, id);
}
